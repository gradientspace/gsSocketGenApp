...
namespace orthogen
{
    ...
    public static partial class OGActions
    {
        ...
        public static bool CanAddDevice()
        {
            return OG.IsInState(OGWorkflow.SocketState) 
                && (OG.TrimLine != null) 
                && OG.Model.HasDevice() == false;
        }

        /// <summary>
        /// initialize device model
        /// </summary>
        public static void AddDevice(SocketModel.ModelModes eMode)
        {
            var M = OG.Model;
            ...
            OG.Model.InitializeDevice(so, eMode);
        ...
        public static void ExportSocketInteractive()
        {
            if (OG.Model.HasDevice() == false)
                return;

            if (ShowExportDialogInEditor || FPlatform.InUnityEditor() == false) {
                FPlatform.GetSaveFileName_Async("Export Socket",
                    Path.Combine(ExportSocketPath, "socket.obj"), new string[] { "*.obj" }, "Mesh Files (*.OBJ)",
                    (filename) => { ExportSocket(filename); });
            } else {
                string filename = Path.Combine(ExportSocketPath, "socket.obj");
                ExportSocket(filename);
            }

        }

        public static void ExportSocket(string sPath)
        {
            if (OG.Model.HasDevice() == false)
                return;
            ...
            StandardMeshWriter.WriteMesh(sPath, SocketMesh, opt);
        }
